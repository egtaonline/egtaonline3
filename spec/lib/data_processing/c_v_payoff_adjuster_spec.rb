...
describe CVPayoffAdjuster do
  describe '.adjust' do
    ...
    context 'when there are no player control variables' do
      context 'and there are no observation control variables' do
        it 'returns the payoff' do
          CVPayoffAdjuster.adjust(payoff, observation_features, observation_cvs, player_features, player_cvs).should == payoff
        end
      end
      context 'and there are observation control variables' do
        ...
        context 'and the features match' do
          let(:observation_features){ { 'feature1' => "5.5", "feature2" => "6.1" } }
          it 'performs the correct adjustment' do
            CVPayoffAdjuster.adjust(payoff, observation_features, observation_cvs,
                player_features, player_cvs).should == payoff + 0.23 * (5.5 - 5.0) + 1.4 * (6.1 - 15.0)
          end
        end
    ...
    context 'when there are player control variables' do
      ...
      context 'and the features match' do
        let(:player_features){ { 'pfeature1' => "4.0", 'pfeature2' => "16.2" } }
        context 'and there are no observation control variables' do
          it 'performs the correct adjustment' do
            CVPayoffAdjuster.adjust(payoff, observation_features, observation_cvs,
                player_features, player_cvs).should == payoff + 0.23 * (4.0 - 5.0) + 1.4 * (16.2 - 15.0)
          end
        end
        context 'and there are observation control variables and matching features' do
          let(:observation_cvs) do
            [double(name: 'feature1', expectation: 5.0, coefficient: 0.23),
             double(name: 'feature2', expectation: 15.0, coefficient: 1.4)]
          end
          ...
          it 'performs the correct adjustment' do
            CVPayoffAdjuster.adjust(payoff, observation_features, observation_cvs,
                player_features, player_cvs).should == payoff + 0.23 * (4.0 - 5.0) + 1.4 * (16.2 - 15.0) + 0.23 * (5.5 - 5.0) + 1.4 * (6.1 - 15.0)
          end
        end
      end
