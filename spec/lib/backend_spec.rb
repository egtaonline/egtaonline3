...
# Needed minute definition to simplify timing tests
class Numeric
  def minutes
    self * 60
  end
...
describe Backend do
  describe 'the SRG configuration works' do
    ...
    before do
      ...
      DRbObject.should_receive(:new_with_uri).with('druby://localhost:30000')
        .and_return(flux_proxy)
      Connection.should_receive(:new).with(proxy: flux_proxy)
        .and_return(connection)
      RemoteSimulationManager.should_receive(:new).with(
        connection: connection,
        simulators_path: '/home/wellmangroup/many-agent-simulations',
        local_data_path: '/mnt/nfs/home/egtaonline/simulations',
        remote_data_path: '/nfs/wellman_ls/egtaonline/simulations',
        flux_active_limit: 90).and_return(simulation_interface)
      RemoteSimulatorManager.should_receive(:new).with(
        connection: connection,
        simulators_path: '/home/wellmangroup/many-agent-simulations')
        .and_return(simulator_interface)
      Backend.configure do |config|
        ...
        config.connection_options[:proxy] = DRbObject.new_with_uri(
          'druby://localhost:30000')
  ...
  describe 'API' do
    ...
    describe 'authenticate' do
      it 'passes the message along to the backend implementation' do
        Backend.configuration.connection.should_receive(:authenticate)
          .with(uniqname: 'fake', verification_number: 123,
                password: 'also_fake').and_return(true)
        Backend.authenticate(uniqname: 'fake', verification_number: 123,
                             password: 'also_fake')
      end
    end
    ...
    describe 'connected?' do
      it 'delegates connected?' do
        Backend.configuration.connection.should_receive(
          :authenticated?).and_return(true)
    ...
    describe 'schedule_simulation' do
      it 'passes the message along to the backend implementation' do
        Backend.configuration.simulation_interface.should_receive(
          :schedule_simulation).with(simulation)
    ...
    describe 'prepare_simulation' do
      it 'passes the message along to the backend implementation' do
        Backend.configuration.simulation_interface.should_receive(
          :prepare_simulation).with(simulation)
    ...
    describe 'clean_simulation' do
      it 'passes the message along to the backend implementation' do
        Backend.configuration.simulation_interface.should_receive(
          :clean_simulation).with(simulation)
    ...
    describe 'prepare_simulator' do
      ...
      it 'passes the message along to the backend implementation' do
        Backend.configuration.simulator_interface.should_receive(
          :prepare_simulator).with(simulator)
