...
describe 'Simulator upload functionality' do
  before :all do
    Simulator.set_callback(:validation, :before, :setup_simulator,
                           if: :source_changed?)
  end
  ...
  describe 'users can update simulators with new programs', type: :feature do
    it 'updates a Simulator with valid simulator' do
      ...
      simulator = Simulator.create!(
        name: 'fake_sim', version: '1.0', email: 'test@example.com',
      ...
      attach_file 'Zipped Source',
                  "#{Rails.root}/spec/support/data/fake_sim_copy.zip"
      Backend.should_receive(:prepare_simulator)
