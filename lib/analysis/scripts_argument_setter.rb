...
class ScriptsArgumentSetter
	...
	def prepare_input(game)
		...
		if subgame_obj != nil
			subgame_obj.prepare_input(game, path_obj.local_input_path, path_obj.subgame_json_name)
		end
	end
...
#{@analysis_obj.set_up_remote_input(File.join(@path_obj.remote_input_path, @path_obj.input_file_name), work_dir)}
		...
		if @subgame_obj != nil
			subgame_set_up_command = "#{@subgame_obj.set_up_remote_script(@path_obj.subgame_script_path,work_dir)}"
			if @subgame_obj.subgame_exist
				subgame_set_up_command += "\n#{@subgame_obj.set_up_remote_input(File.join(@path_obj.remote_subgame_path, @path_obj.subgame_json_file_name), work_dir)}"
			end
...
export PYTHONPATH=$PYTHONPATH:#{@path_obj.scripts_path}
	...
	def get_script_command
		if @reduction_obj !=nil && @subgame_obj !=nil && @subgame_obj.subgame_exist
			
			<<-DOCUMENT
#{@reduction_obj.run_with_option(@path_obj.input_file_name, @path_obj.reduction_file_name)}
#{@subgame_obj.run_with_option(@path_obj.reduction_file_name, @path_obj.subgame_json_file_name,"-k #{@path_obj.subgame_json_file_name}")}
#{@analysis_obj.run_with_option(@path_obj.reduction_file_name, @path_obj.output_file_name,"-sg #{@path_obj.subgame_json_file_name}")}
			DOCUMENT
		
		elsif @reduction_obj !=nil && @subgame_obj !=nil && !(@subgame_obj.subgame_exist)
			
			<<-DOCUMENT
#{@reduction_obj.run_with_option(@path_obj.input_file_name, @path_obj.reduction_file_name)}
#{@subgame_obj.run_with_option(@path_obj.reduction_file_name, @path_obj.subgame_json_file_name)}
#{@analysis_obj.run_with_option(@path_obj.reduction_file_name,@path_obj.output_file_name, "-sg #{@path_obj.subgame_json_file_name}")}
			DOCUMENT
			...
			<<-DOCUMENT
#{@subgame_obj.run_with_option(@path_obj.input_file_name, @path_obj.subgame_json_file_name,"-k #{@path_obj.subgame_json_file_name}")}
#{@analysis_obj.run_with_option(@path_obj.input_file_name, @path_obj.output_file_name,"-sg #{@path_obj.subgame_json_file_name}")}
			...
			<<-DOCUMENT
#{@subgame_obj.run_with_option(@path_obj.input_file_name, @path_obj.subgame_json_file_name)}
#{@analysis_obj.run_with_option(@path_obj.input_file_name, @path_obj.output_file_name,"-sg #{@path_obj.subgame_json_file_name}")}
			DOCUMENT
...
#{@analysis_obj.run_with_option(@path_obj.reduction_file_name, @path_obj.output_file_name)}		
	...
	def clean_up_remote_command
		analysis_clean_up = @analysis_obj.get_output(@path_obj.working_dir, @path_obj.output_file_name, @path_obj.local_input_path)
		if @subgame_obj != nil
