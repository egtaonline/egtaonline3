  ...
  describe '#clean' do
    ...
    context 'when connected to flux' do
      ...
      it 'requests the old files be deleted' do
        flux_proxy.should_receive(:exec!).with(
          "rm -rf #{simulators_path}/#{simulator.fullname}*;" \
          " rm -rf #{simulators_path}/#{simulator.name}.zip")
          .and_return('fake')
        expect(cleaner.clean(connection, simulator)).to eq('fake')
      end
    end
    ...
    context 'when not connected to flux' do
      ...
      it 'informs the caller that the connection was broken' do
        expect { cleaner.clean(connection, simulator) }
          .to raise_error('Connection broken.')
      end
    end
  end
end
