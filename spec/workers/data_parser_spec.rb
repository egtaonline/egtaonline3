...
describe DataParser do
  describe '#perform' do
    ...
    before do
      ...
      Dir.should_receive(:entries).with("fake/path").and_return(['first_observation.json', 'not_valid.json'])
      ObservationProcessor.should_receive(:new).with(simulation, ['fake/path/first_observation.json']).and_return(observation_processor)
    end
