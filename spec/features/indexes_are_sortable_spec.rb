...
feature 'index pages are sortable' do
  ...
  scenario 'when on the simulator index page' do
    create(:simulator, version: 'alpha')
    create(:simulator, version: 'beta')
  ...
  %w(game_scheduler deviation_scheduler dpr_deviation_scheduler
     dpr_scheduler generic_scheduler hierarchical_deviation_scheduler
     hierarchical_scheduler).each do |scheduler|
    scenario 'some schedulers' do
      first_name, second_name = [
        create(scheduler.to_sym).simulator_fullname,
  ...
  scenario 'some simulations' do
    create(:simulation, state: 'queued')
    create(:simulation, state: 'pending')
