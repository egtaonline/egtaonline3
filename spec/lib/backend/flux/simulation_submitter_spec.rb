...
describe SimulationSubmitter do
  ...
  describe '#submit' do
    ...
    context 'when no exceptions are raised' do
      before do
        proxy.should_receive(:exec!).with(
          "qsub -V -r n #{remote_data_path}/#{simulation.id}/wrapper")
          .and_return(value)
      end
      ...
      context 'when the submission is a success' do
        ...
        it 'moves the simulation to queued with job id from the submission' do
          simulation.should_receive(:queue_as).with(123_534_123)
    ...
    context 'when an exception is raised' do
      ...
      before do
        proxy.stub(:exec!).with(
          "qsub -V -r n fake/remote/path/#{simulation.id}/wrapper")
          .and_raise('Failure')
      end
