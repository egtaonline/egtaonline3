...
class AnalysisManager 
  def initialize(game_id,enable_reduced,analysis_hash,reduced_num_array,roles_count)
  ...
  def set_script_arguments
    @running_script_command = ScriptsArgumentSetter.scriptCommand(@enable_reduced, @analysis_hash,@reduced_num_array, @roles_count, @path_finder)
  end

  def create_pbs
     @pbs = AnalysisPbsFormatter.new(@path_finder, @running_script_command )
  end
  def submit_job
