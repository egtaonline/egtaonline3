...
describe 'waiting for the files to show up' do
  context 'error file is missing' do
    ...
    let!(:simulation){ create(:simulation, state: 'queued', size: 2, profile: profile, id: 3) }
    let!(:scheduling_requirement){ create(:scheduling_requirement, profile: simulation.profile) }
